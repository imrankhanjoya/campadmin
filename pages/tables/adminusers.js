...
export default function Brands({ adminuserslist }) {
  return (
    <div>
          ...
          <div className="sm:p-7 p-4">
            ...
            <table className="w-full text-left">
              ...
              <tbody className="text-gray-600 dark:text-gray-100">
                {adminuserslist.map((e, index )=>{
                  return(<tr key={index}>
                  <td className="sm:p-3 py-2 px-1 border-b border-gray-200 dark:border-gray-800 text-gray-800">{e.index}</td>
...
export async function getServerSideProps() {
  const response = await axios.get("http://localhost:3000/api/adminuserapi");
